...
logdir = '/Users/siddharthgoel/code/misc/db-graph/hbase/'
...
def display(statsList):
	for stats in statsList:
		print "Threads = " + str(stats.threads) + "\tExpThr = " + str(stats.expThr) +\
			...
			"\tAvgUpdate = " + str(stats.avgUpdateLat) + "\tAvgRead = " + str(stats.avgReadLat)
...
def achThrExpThrGraph(statsList):
	...
	pyplot.title('Plot of throughput attained in hbase with various threads running 500000 operation counts')
	pyplot.legend(loc=2)
	st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d-%H-%M-%S')
	save("Hbase-throughput-graph-"+str(st), ext="png", close=False, verbose=True)
	#pyplot.show()
...
def avgReadExpThrGraph(statsList):
	...
	pyplot.axis([0, 1.1 * max(expThr) , 0, 50000.0 ])
	pyplot.title('Plot of Average Read Latency against Target Throughput in hbase with various threads running about 250000 read operations.')
	pyplot.legend(loc=2)
	st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d-%H-%M-%S')
	save("Hbase-read-latency-graph-"+str(st), ext="png", close=False, verbose=True)
	#pyplot.show()
...
def avgUpdateExpThrGraph(statsList):
	...
	pyplot.axis([0, 1.1 * max(expThr) , 0, 10000.0 ])
	pyplot.title('Plot of Average Update Latency against Target Throughput in hbase with various threads running about 250000 update operations')
	pyplot.legend(loc=2)
	st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d-%H-%M-%S')
	save("Hbase-update-latency-graph-"+str(st), ext="png", close=False, verbose=True)
	#pyplot.show()
...
def main():
	...
	for subdir, dirs, files in os.walk(logdir):
		for file in files:
			...
			runtime = parseLastValue( linecache.getline(filePath, 3) )
			achievedThroughput = parseLastValue( linecache.getline(filePath, 4) )
			averageUpdateLatency = parseLastValue( linecache.getline(filePath, 6) )
			averageReadLatency = parseLastValue( linecache.getline(filePath, 1014) )
